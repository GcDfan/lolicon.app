	...
	$.fn._parallax =
		browser.name == 'ie' || browser.name == 'edge' || browser.mobile
			...
			: function(middle = 50) {
					...
					if (this.length > 1) {
						for (var i = 0; i < this.length; i++) $(this[i])._parallax();
					...
					$this.each(function() {
						...
						on = function() {
							$t.css('background-position', 'center ' + middle + '%');

							$window.on('scroll._parallax', function() {
								var pos = parseInt($window.scrollTop()) - parseInt($t.position().top);

								$t.css('background-position', 'center ' + ((pos / $t.outerHeight()) * (100 - middle) + middle) + '%');
							});
						};
